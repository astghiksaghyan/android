...
public class SecondaryChooserFragment extends android.app.DialogFragment {
    ...
    private List<String> mCustomStoragesList;
    private SecondaryChooserAdapter mSecondaryChooserAdapter;
    ...
    private View.OnClickListener mSelectButtonClickListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            if (mConfig.ismActionSave()) {
                DiskUtil.saveChooserPathPreference(mConfig.getmPreference(), theSelectedPath);
            } else {
                Log.d("StorageChooser", "Chosen path: " + theSelectedPath);
            }

            StorageChooser.mOnSelectListener.onSelect(theSelectedPath);
    ...
    private View.OnClickListener mCreateButtonClickListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            if (validateFolderName()) {
                ...
                if (success) {
                    Toast.makeText(mContext, mContent.getmFolderCreatedToastText(), Toast.LENGTH_SHORT).show();
                    trimPopulate(theSelectedPath);
                    hideKeyboard();
                    hideAddFolderView();
                } else {
                    Toast.makeText(mContext, mContent.getmFolderErrorToastText(), Toast.LENGTH_SHORT).show();
                }
            }
        }
    };
    private AdapterView.OnItemClickListener mSingleModeClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> adapterView, View view, final int i, long l) {
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (i < mCustomStoragesList.size()) {
                        String jointPath = theSelectedPath + "/" + mCustomStoragesList.get(i);
                        if (FileUtil.isDir(jointPath)) {
                            populateList("/" + mCustomStoragesList.get(i));
                        } else {
                            StorageChooser.mOnSelectListener.onSelect(jointPath);
                            dissmissDialog(FLAG_DISSMISS_NORMAL);
                        }
                    }
                }
    ...
    private AdapterView.OnItemLongClickListener mLongClickListener = new AdapterView.OnItemLongClickListener() {
        ...
        public boolean onItemLongClick(AdapterView<?> adapterView, View view, int i, long l) {
            ...
            if (!FileUtil.isDir(jointPath)) {
                MODE_MULTIPLE = true;
                mListView.setOnItemClickListener(mMultipleModeClickListener);
    ...
    private View.OnClickListener mMultipleModeDoneButtonClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View view) {
            StorageChooser.mOnMultipleSelectListener.onDone(mMultipleModeList);
            bringBackSingleMode();
            dissmissDialog(FLAG_DISSMISS_NORMAL);
        }
    };
...
//        mNewFolderButton.setImageDrawable(ContextCompat.getDrawable(mContext, R.drawable.window_close));
        ...
        SecondaryChooserAdapter.mShouldEnable = false;
    }
    ...
    private void hideAddFolderView() {
        ...
        SecondaryChooserAdapter.mShouldEnable = true;
...
//        mNewFolderButton.setImageDrawable(ContextCompat.getDrawable(mContext, R.drawable.plus));
    ...
    private void performBackAction() {
        ...
        if (slashIndex != -1) {
            if (MODE_MULTIPLE) {
                bringBackSingleMode();
                mSecondaryChooserAdapter.notifyDataSetChanged();

            } else {
                if (!mConfig.ismSkipOverview()) {
    ...
    private void dissmissDialog(int flag) {
        ...
        switch (flag) {
            case FLAG_DISSMISS_INIT_DIALOG:
                ...
                c.show(mConfig.getmFragmentManager(), "storagechooser_dialog");
                break;
    ...
    private View getLayout(LayoutInflater inflater, ViewGroup container) {
        mConfig = StorageChooser.mSConfig;
        mScheme = mConfig.getmScheme();
        mHandler = new Handler();

        // init storage-chooser content [localization]
        if (mConfig.getmContent() == null) {
            mContent = new Content();
        } else {
            mContent = mConfig.getmContent();
        }
        ...
        initListView(mContext, mLayout, mConfig.ismShowMemoryBar());
    ...
    private void initUI() {
        ...
        mNewFolderView.setBackgroundColor(mScheme[Theme.SEC_FOLDER_CREATION_BG_INDEX]);
        ...
        (mLayout.findViewById(R.id.secondary_container)).setBackgroundColor(mScheme[Theme.SEC_BG_INDEX]);
    ...
    private void updateUI() {
        ...
        mFolderNameEditText.setHint(mContent.getmTextfieldHintText());

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mFolderNameEditText.setHintTextColor(mScheme[Theme.SEC_HINT_TINT_INDEX]);
        }


        // set label of buttons [localization]
        mSelectButton.setText(mContent.getmSelectLabel());
        mCreateButton.setText(mContent.getmCreateLabel());


        // set colors
        mSelectButton.setTextColor(mScheme[Theme.SEC_SELECT_LABEL_INDEX]);
        mPathChosen.setTextColor(mScheme[Theme.SEC_ADDRESS_TINT_INDEX]);

        // set addressbar typeface
        if (mConfig.getmHeadingFont() != null) {
            mPathChosen.setTypeface(ChooserDialogFragment.getSCTypeface(mContext,
                    mConfig.getmHeadingFont(), mConfig.ismHeadingFromAssets()));
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mNewFolderImageView.setImageTintList(ColorStateList.valueOf(mScheme[Theme.SEC_ADDRESS_TINT_INDEX]));
            mBackButton.setImageTintList(ColorStateList.valueOf(mScheme[Theme.SEC_ADDRESS_TINT_INDEX]));
        }
        mMultipleOnSelectButton.setColor(mScheme[Theme.SEC_DONE_FAB_INDEX]);
        mLayout.findViewById(R.id.custom_path_header).setBackgroundColor(mScheme[Theme.SEC_ADDRESS_BAR_BG]);
        ...
        if (mConfig.getmSecondaryAction().equals(StorageChooser.FILE_PICKER)) {
    ...
    /**
     * storage mListView related code in this block
     */
    private void initListView(Context context, View view, boolean shouldShowMemoryBar) {
        mListView = view.findViewById(R.id.storage_list_view);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mFilesProgress.setIndeterminateTintList(ColorStateList.valueOf(mScheme[Theme.OVERVIEW_MEMORYBAR_INDEX]));
        }
        ...
        mSecondaryChooserAdapter = new SecondaryChooserAdapter(mCustomStoragesList, context, mScheme,
                mConfig.getmListFont(), mConfig.ismListFromAssets());
        mSecondaryChooserAdapter.setmPrefixPath(theSelectedPath);

        mListView.setAdapter(mSecondaryChooserAdapter);
        //listview should be clickable at first
        SecondaryChooserAdapter.mShouldEnable = true;
        mListView.setOnItemClickListener(mSingleModeClickListener);

        if (isFilePicker && mConfig.ismMultiSelect()) {
    ...
    private void handleListMultipleAction(int i, View view) {
        ...
        if (!mSecondaryChooserAdapter.mSelectedPaths.contains(i)) {
            view.setBackgroundColor(mResourceUtil.getPrimaryColorWithAlpha());

            mSecondaryChooserAdapter.mSelectedPaths.add(i);
            mMultipleModeList.add(jointPath);

        } else {
            //this item is selected before
            mSecondaryChooserAdapter.mSelectedPaths.remove(mSecondaryChooserAdapter.mSelectedPaths.indexOf(i));
            // reset bg to white
            view.setBackgroundColor(mScheme[Theme.SEC_BG_INDEX]);
        ...
        if (mListView.getOnItemLongClickListener() != null && MODE_MULTIPLE)
...
//        for(int j=0; j<mSecondaryChooserAdapter.mSelectedPaths.size(); j++) {
//            if()
//        }
//    }
    ...
    private void bringBackSingleMode() {
        ...
        mSecondaryChooserAdapter.mSelectedPaths.clear();
    ...
    private void evaluateAction(int i) {
        String preDefPath = mConfig.getmPredefinedPath();
        boolean isCustom = mConfig.ismAllowCustomPath();
        if (preDefPath == null) {
    ...
    private void populateList(String path) {
        if (mCustomStoragesList == null) {
            mCustomStoragesList = new ArrayList<>();
        } else {
            mCustomStoragesList.clear();
        }

        mFileUtil = new FileUtil();
        theSelectedPath = theSelectedPath + path;
        if (mSecondaryChooserAdapter != null && mSecondaryChooserAdapter.getmPrefixPath() != null) {
            mSecondaryChooserAdapter.setmPrefixPath(theSelectedPath);
        }
        ...
        if (isFilePicker) {
            if (mConfig.ismCustomFilter()) {
                ...
                if (mConfig.getmSingleFilter() != null) {
                    new FileFilterTask(mConfig, false).execute();
                } else {
                    volumeList = mFileUtil.listFilesInDir(theSelectedPath);
            ...
            volumeList = mFileUtil.listFilesAsDir(theSelectedPath);
            setAdapterList(volumeList);
            refreshList();
            setBundlePathOnUpdate();
        }
    ...
    public void setBundlePathOnUpdate() {
        if (mConfig.ismResumeSession() && StorageChooser.LAST_SESSION_PATH != null) {
    ...
    public void refreshList() {
        if (mSecondaryChooserAdapter != null) {
            mSecondaryChooserAdapter.notifyDataSetChanged();
        }
    }
    ...
    public void setAdapterList(File[] volumeList) {
        if (volumeList != null) {
            for (File f : volumeList) {
                if (mConfig.ismShowHidden()) {
                    mCustomStoragesList.add(f.getName());
                } else {
            ...
            Collections.sort(mCustomStoragesList, new Comparator<String>() {
                @Override
    ...
    private void trimPopulate(String s) {
        ...
        if (mSecondaryChooserAdapter != null) {
            mSecondaryChooserAdapter.setmPrefixPath(s);
            mSecondaryChooserAdapter.notifyDataSetChanged();
        }
    }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        Dialog d = StorageChooser.mDialog;
        if (getActivity() != null) {
    ...
    public void onCancel(DialogInterface dialog) {
        ...
        StorageChooser.mOnCancelListener.onCancel();
    }
    ...
    private boolean validateFolderName() {
        if (mFolderNameEditText.getText().toString().trim().isEmpty()) {
            mFolderNameEditText.setError(mContent.getmTextfieldErrorText());
            return false;
        }
        return true;
    }
        ...
        protected Boolean doInBackground(Void... voids) {
            if (isMultiple) {
                UniversalFileFilter universalFileFilter =
                        new UniversalFileFilter(mConfig.ismCustomFilter(), (ArrayList<String>) mConfig.getmCustomEnum());
                fileList = new File(theSelectedPath)
                        .listFiles(universalFileFilter);
            } else {
                fileList = new File(theSelectedPath).listFiles(new UniversalFileFilter(mConfig.getmSingleFilter()));
            }
            return true;
        }
